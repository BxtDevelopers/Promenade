...
export default function ReviewsSection() {
  ...
  return (
    <section id="reviews" className="bg-white py-24 lg:py-32 overflow-hidden relative">
      {/* Abstract Background Shapes for Distinctiveness */}
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div className="absolute -top-[20%] -left-[10%] w-[50vw] h-[50vw] rounded-full bg-[#FDE8D8]/30 blur-[100px]" />
        <div className="absolute top-[60%] -right-[10%] w-[40vw] h-[40vw] rounded-full bg-[#ffa07a]/5 blur-[80px]" />
      </div>
      ...
      <div className="w-[92%] max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-20">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold font-['Poppins'] text-[#1A365D] mb-8 tracking-tight">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 overflow-hidden py-4">
            {[0, 1, 2].map((offset) => {
              ...
              return (
                <div
                  key={`${idx}-${current}`}
                  className={`bg-white rounded-[2rem] p-10 border border-[#1A365D]/5 shadow-[0_10px_30px_rgba(26,54,93,0.05)] hover:shadow-[0_20px_40px_rgba(26,54,93,0.12)] hover:-translate-y-2 transition-all duration-500 relative flex flex-col h-full group ${offset === 1 ? 'hidden md:flex' : ''} ${offset === 2 ? 'hidden lg:flex' : ''}`}
                >
        ...
        <div className="mt-16 flex flex-col md:flex-row items-center justify-center gap-6">
          <a href="#" className="group flex items-center gap-4 bg-white px-8 py-5 rounded-2xl border border-[#1A365D]/5 shadow-[0_10px_30px_rgba(26,54,93,0.06)] hover:shadow-[0_15px_40px_rgba(26,54,93,0.12)] hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
              <p className="text-[16px] font-bold text-[#1A365D] font-['Poppins']">Read Our Google Reviews</p>
              <div className="flex items-center gap-1 mt-0.5">
                {[...Array(5)].map((_, i) => <Star key={i} className="w-3.5 h-3.5 text-[#F59E0B] fill-[#F59E0B]" />)}
                <span className="text-[14px] text-[#1A365D]/60 font-['DM_Sans'] font-medium ml-1">4.9/5 from Verified Patients</span>
              </div>
            </div>
          </a>
        </div>
